...
Two-phase: scan and propose, then fix only what the user approves. The highest-yield category is usually _dead code left over from a removed feature_ — type-clean deletion routinely leaves stale UI copy, dead nav entries, dead prompts, and orphan helpers behind. Sweep that aggressively.
