---
name: ci-check
description: Set up or audit a lightweight CI pipeline (GitHub Actions etc.) running typecheck/lint/build on every push — earlier failure signal than the deploy-time build. Build/lint/typecheck gating now; test-gating deferred until a green suite exists (/test first). NOT running checks locally (use `npm run verify`); NOT the pre-deploy risk review (use /deploy-check); NOT diagnosing a broken build (use /fix-bug).
---
