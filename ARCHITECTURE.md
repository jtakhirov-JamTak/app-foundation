...
- Lab Lighthouse gate (`lighthouserc.cjs`, production build, mobile emulation): `/sign-in` LCP ≤ 2.5 s; `/` LCP ≤ 3.4 s — unauthenticated `/` client-redirects to `/sign-in`, so its lab LCP includes the redirect hop and does not represent the signed-in shell cold start (calibrated 2026-07-24 against the first real Lighthouse run, median + ~10%). CLS ≤ 0.1 and TBT ≤ 200 ms on both URLs. Lab TBT stands in for INP, which only exists as a field metric.
